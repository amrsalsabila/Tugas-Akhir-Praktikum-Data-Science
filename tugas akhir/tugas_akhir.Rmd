...
```{r }
d <- read.csv("E:/kuliah/semester 5/prak ds/tugas akhir/ReviewpeduliLindungi.csv")
# remove spam 
...
write.csv(dataframe,file = "peduliCleann.csv")
...
```
 ...
 Data Labelling
```{r}
peduliClean<-read.csv("peduliCleann.csv",header=TRUE)
View(tiketClean)
...
hasil = score.sentiment(peduliClean$text, kata.positif, kata.negatif)
...
#View(data)
write.csv(data, file = "peduliLabell.csv")
```


 Tampilkan data setelah diberi label
```{r}
data=read.csv("peduliLabell.csv")
...
```{r}
ui <- fluidPage(
    titlePanel("Sentiment Analysis Review Pengguna Peduli Lindungi"),
        mainPanel(
...
server <- function(input, output) {
  ...
  output$tbl1 = DT::renderDataTable({
    datatabel <-read.csv("peduliLabell.csv",stringsAsFactors = FALSE)
    DT::datatable(datatabel, options= list(lengthChange = FALSE))
    })
    
    #output Bar Plot
  output$scatterplot <- renderPlot({
    d<-read.csv("peduliLabell.csv",stringsAsFactors = FALSE) 
  ...
  output$Wordcloud <- renderPlot({
    require (corpus)
    df<-read.csv("peduliCleann.csv",stringsAsFactors = FALSE)
    glimpse(df)
    set.seed(20)
